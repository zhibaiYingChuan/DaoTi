...
**🚀 [道体在线演示](https://sfang.cc/#demos)** | **⚡ [脉冲编码接口演示](https://modelscope.cn/studios/spring30/daoti-v53-spike)** | **📖 [论文](docs/papers/)** | **📄 [白皮书](docs/白皮书_道体基座技术.md)** | **💬 [讨论](https://github.com/zhibaiYingChuan/DaoTi/issues)**
...
# 3b. 或运行命令行演示
python scripts/demo_real_text.py
```
...
DaoTi/
├── daoti/                       # 📁 核心包
│   ├── __init__.py              #   包入口（re-export 公开接口）
│   ├── inference.py             #   推理接口（加载 + 预测 + 适配器）
│   ├── _model_core.py           #   模型架构（推理构建版，受许可证保护）
│   ├── _constants.py            #   数据常量（卦象、五行、知识库等）
│   └── spike_interface.py       #   64 通道脉冲编码接口
├── app/                         # 📁 演示应用
│   ├── app.py                   #   Gradio 交互式演示
│   ├── app_spike.py             #   脉冲编码接口演示
│   └── requirements_spike.txt   #   脉冲演示依赖
├── scripts/                     # 📁 工具脚本
│   ├── train_adapter.py         #   领域适配器训练（冻结道体，只训练 LoRA）
│   ├── train_physics_adapter.py #   物理参数适配器训练
│   ├── build_tokenizer.py       #   分词器构建（推理用，非训练分词器）
│   ├── eval_benchmark.py        #   基准测试
│   ├── demo_real_text.py        #   命令行推理演示
│   └── check_training_leak.py   #   训练逻辑泄露检测
├── weights/                     # 📁 模型权重
│   ├── yijing_v53_daoti.pt      #   模型权重
│   ├── yijing_v53_daoti.pt.sha256
│   ├── daoti_full.pt            #   完整检查点
│   ├── daoti_v53_tokenizer.pt   #   分词器
│   └── yijing_v53_config.json   #   模型配置
├── docs/                        # 📁 文档
│   ├── 白皮书_道体基座技术.md
│   ├── API_REFERENCE.md
│   ├── ADAPTER_TRAINING.md
...
```python
import torch
from daoti.inference import load_daoti, predict, verify_sha256
...
# ——▶ Step 2: 加载模型（CPU 模式）——▶
model = load_daoti("weights/yijing_v53_daoti.pt", device='cpu')
...
| 层级 | 机制 | 说明 |
|:---|:---|:---|
| **第一道门槛** | 领域分类器 | 任何输入必须先经过领域判定。未知领域 / 低置信度 → 拒绝执行 |
...
```python
from daoti.inference import load_daoti, generate_response
import torch
...
```bash
# 1. 准备数据 (JSONL格式)
echo '{"text": "光栅周期为200纳米时，共振波长出现在550纳米附近", "domain": "optics"}' > my_data.jsonl
...
输出 `benchmarks/benchmark_results.json`，包含：八宫分类准确率、64 卦原型检索 top-1/top-5 准确率、相干性分布统计、随机基线对比。详见 [BENCHMARK.md](docs/BENCHMARK.md)。

## 保护策略

| 层级 | 内容 | 状态 |
|:---|:---|:---|
| 理论层 | 退化基态理论、规范场论结构、不动点性质 | ✅ 论文+白皮书已公开 |
...
| 附 | 道体基座技术白皮书 | Technical White Paper |
...
- **DaoTi Research License v1.0**：允许推理使用，**禁止**以下行为：
  - 对模型权重进行逆向工程以提取训练方法
  - 使用权重训练或蒸馏竞争模型
  - 将模型权重作为独立产品再分发或转售
...
| 共振腔 | 每域维护的 EMA 中心向量 + 波能量 + 相干度 | 模型判断「我懂不懂」的自我评估系统 |
