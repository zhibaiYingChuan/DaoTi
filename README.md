...
# 3. Run prediction
text_ids = torch.randint(1, 8145, (1, 256))  # your tokenized text
...
- **CPU-trained**: 完整5.06M参数模型在消费级CPU上训练完成
- **结构性增效**: 以数量级更低的计算资源达到100M+参数模型的效果
- **双通道融合**: 文本理解 + 易经符号推理
- **冻结道体**: 核心编码器权重为不变量（退化基态）
- **7项符号推理**: 易经专业任务平均准确率 99.96%
- **三种占法**: 周易、梅花、六爻
- **卦象检索**: 文本到卦象的64维原型空间映射
