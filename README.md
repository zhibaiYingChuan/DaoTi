...
| 脚本 | 用途 | 输入 |
|:---|:---|:---|
| `inference.py` | **极简演示**：验证模型加载和推理接口可用 | 随机 token ids（功能验证用） |
| `demo_real_text.py` | **完整演示**：体验从真实中文输入到完整推理结果的全流程 | 预置真实中文文本，输出完整推理链条 |

```bash
# 极简演示（验证模型能否正常加载和推理）
python inference.py

# 完整演示（看到真实的中文输入 → 结构化推理输出）
python demo_real_text.py
```

`demo_real_text.py` 会逐步骤展示：文本编码 → 领域判定 → 洛书空间状态 → 八宫/六亲/天干地支/旺相休囚等全部输出。每一步都有 `print` 说明，跑完就能看到完整的推理链条。
