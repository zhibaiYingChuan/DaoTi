...
def fill_tokens(tokens, max_seq=256):
...
def eval_full(model, proto_n, char_to_id, device):
    top1 = 0
    top5 = 0
    sims = []
    cohs = []
    for gi in range(64):
        tokens = [char_to_id.get(ch, 1) for ch in GUA_REAL_TEXTS[gi]]
        tokens = fill_tokens(tokens)
        ...
        if int(sim.argmax().item()) == gi:
            top1 += 1
        if gi in sim.topk(5).indices.tolist():
            top5 += 1
...
def train_bpe(texts, num_merges=500):
    word_freqs = Counter()
    for text in texts:
        word = tuple(text)
        word_freqs[word] += 1
...
def main():
    print("=" * 60)
    print("  DaoTi V53 Tokenizer Builder")
    print("=" * 60)
    ...
    threshold = np.percentile(norms[1:], 75)
    ...
    print("\n  [Phase 2] Character-level mapping (V3 best-match)...")
    char_to_id = {}
    used_tokens = set()
    ...
    t1, t5, sims, cohs = eval_full(model, proto_n, char_to_id, device)
    print(f"    Top-1={100*t1/64:.1f}% ({t1}/64)  Top-5={100*t5/64:.1f}% ({t5}/64)  Coh>0.7={sum(1 for c in cohs if c>0.7)}")

    print("\n  [Phase 3] BPE training...")
    merges = train_bpe(GUA_REAL_TEXTS, num_merges=500)
    print(f"    Learned {len(merges)} BPE merges")

    bpe_vocab = set()
    for text in GUA_REAL_TEXTS:
        tokens = apply_bpe(text, merges)
        bpe_vocab.update(tokens)
    bpe_vocab = sorted(bpe_vocab, key=lambda t: -sum(text.count(t) for text in GUA_REAL_TEXTS))
    print(f"    BPE vocabulary: {len(bpe_vocab)} tokens")

    print("\n  [Phase 4] Mapping BPE tokens to model vocabulary...")
    bpe_to_id = {}
    for bpe_idx, bpe_token in enumerate(bpe_vocab):
        if bpe_idx % 100 == 0:
            print(f"    {bpe_idx+1}/{len(bpe_vocab)}...")
        if len(bpe_token) == 1:
            bpe_to_id[bpe_token] = char_to_id.get(bpe_token, 1)
            continue
        guas_with_token = set()
        for gi, text in enumerate(GUA_REAL_TEXTS):
            if bpe_token in text:
                guas_with_token.add(gi)
        if not guas_with_token:
            bpe_to_id[bpe_token] = 1
            continue
        target = np.zeros(64, dtype=np.float32)
        for gi in guas_with_token:
            target[gi] = 1.0
        best_tid = None
        best_score = -1
        for tid in active_ids:
            matched_gua = token_to_best_gua.get(tid, -1)
            if matched_gua in guas_with_token:
                score = token_to_max_sim.get(tid, 0) + 0.5
            else:
                score = np.dot(token_profiles[tid], target)
            if score > best_score:
                best_score = score
                best_tid = tid
        if best_tid is not None:
            bpe_to_id[bpe_token] = best_tid
        else:
            bpe_to_id[bpe_token] = 1

    print("\n  [Phase 5] Greedy per-character refinement...")
    ...
    for round_num in range(5):
        ...
        for gi in range(64):
            ...
            tokens = [best_char_to_id.get(ch, 1) for ch in text]
            tokens_full = fill_tokens(tokens)
            text_ids = torch.tensor([tokens_full], dtype=torch.long, device=device)
            with torch.no_grad():
                text_feat = model.encode_text(text_ids)
                feat_n = F.normalize(text_feat, p=2, dim=-1)
            ...
            for ch in unique_chars:
                current_tid = best_char_to_id.get(ch, 1)
                guas_with_ch = set(char_to_guas.get(ch, []))
                candidates = []
                for tid in active_ids:
                    matched_gua = token_to_best_gua.get(tid, -1)
                ...
                top_candidates = [tid for _, tid in candidates[:5]]
                ...
                for cand_tid in top_candidates:
                    if cand_tid == current_tid:
                        continue
                    test_tokens = [best_char_to_id.get(c, 1) if c != ch else cand_tid for c in text]
                    test_full = fill_tokens(test_tokens)
                    test_ids = torch.tensor([test_full], dtype=torch.long, device=device)
                    with torch.no_grad():
                        test_feat = model.encode_text(test_ids)
                        test_n = F.normalize(test_feat, p=2, dim=-1)
                        new_sim = float(torch.mm(test_n, proto_n[gi:gi+1].t()).item())
                    if new_sim > best_s:
                        best_s = new_sim
                        best_tid = cand_tid

                if best_tid != current_tid:
                    best_char_to_id[ch] = best_tid
                    changed += 1

        t1r, t5r, simsr, cohsr = eval_full(model, proto_n, best_char_to_id, device)
        print(f"    Round {round_num+1}: changed={changed}  Top-1={100*t1r/64:.1f}% ({t1r}/64)  Top-5={100*t5r/64:.1f}% ({t5r}/64)  Coh>0.7={sum(1 for c in cohsr if c>0.7)}")
        if t1r >= best_top1:
            best_top1 = t1r
        if changed == 0:
            break
    ...
    print("\n  [Final Evaluation]...")
    t1f, t5f, simsf, cohsf = eval_full(model, proto_n, best_char_to_id, device)
    print(f"\n  Top-1: {100*t1f/64:.1f}% ({t1f}/64)")
    print(f"  Top-5: {100*t5f/64:.1f}% ({t5f}/64)")
    print(f"  AvgSim: {np.mean(simsf):.4f}")
    print(f"  Coherence mean: {np.mean(cohsf):.4f}  >0.7: {sum(1 for c in cohsf if c>0.7)}/64")
    ...
    tokenizer_data = {
        ...
        'max_seq': MAX_SEQ,
        'version': 'v53-v5',
        'method': 'probe+bpe+refinement',
    }
    torch.save(tokenizer_data, 'daoti_v53_tokenizer.pt')
    print(f"\n  Saved daoti_v53_tokenizer.pt ({len(best_char_to_id)} chars, {len(merges)} BPE merges)")

    with open("optimized_tokenizer.json", "w", encoding="utf-8") as f:
        json.dump(best_char_to_id, f, ensure_ascii=False, indent=2)
